    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            if (state === 'Admin') {
                ...
                else{ // it means success is false
                    toast.error(data.message)
                }
            }
            else{ // it means we are in Doctors State and we will call doctor login api
    ...
    return (
        <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
            ...
            border rounded-xl text-[#5E5E5E] text-sm shadow-lg'>
                ...
                <button className='bg-[#5F6FFF] text-white w-full py-2 rounded-md '>Login</button>
                {
                    state === 'Admin' 
